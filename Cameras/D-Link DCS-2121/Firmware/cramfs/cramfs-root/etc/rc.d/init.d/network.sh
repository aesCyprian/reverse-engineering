...
# After radio_on need sleep 3 second to guarantee driver ready to used.
	echo -n "\
...
AutoIPOK() {
	[ "$AutoIPEnable_byte" -eq 0 ] && return 1
	echo "zcip action"
	/etc/rc.d/init.d/zcip.sh restart
	return 0
}
...
#echo "== lo =="
	...
	echo "== $median =="
	ifup $median 
	dhcpOK $median || AutoIPOK $median || ifup -f $median=fallback
...
stop() {
	...
	probeWireless && iwpriv ra0 radio_on
